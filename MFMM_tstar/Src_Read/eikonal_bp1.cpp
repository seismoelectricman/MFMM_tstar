...
CalculateDis3DResult CalculateDistance3D(double *T, double *TS, double Fijk, double Qijk, int *dims, int i, int j, bool usesecond, bool usecross, bool *Frozen);
	...
	for (itt = 0; itt < (npixels); itt++) { /* */
		...
		for (w = 0; w < 6; w++) {
			...
			if (isntfrozen3d(i, j, k, dims, Frozen)) {
				...
				if ((T[IJK_index] > -1) && T[IJK_index] <= listprop[0]) {  // T[IJK_index] > -1 - indicates that this node has an existing Tt (T is not -1).
