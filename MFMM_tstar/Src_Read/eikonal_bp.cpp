...
void msfm2dCpp(double *T, double *F, int izsrcn, int ixsrcn, int *dims, bool usesecond, bool usecross) { // bool-This keyword is a built-in data type in C++ that can 
	...
	initialize_list(listval, listprop); // initialize_list, to initialize the 2D array listval and the 1D array listprop. -wdd 
	neg_listv = listval[listprop[1] - 1]; //assigns neg_listv the pointer to the row in listval specified by the index listprop[1] - 1. -wdd
