...
CalculateDis3DResult CalculateDistance3D(double *T, double *TS, double Fijk, double Qijk, int *dims, double dzxy, int i, int j, int k, bool usesecond, bool usecross, bool *Frozen) {
	...
	in = i + 0; jn = j - 1; kn = k + 0; if (isfrozen3d(in, jn, kn, dims, Frozen)) {
		Tym1 = T[mindex3(in, jn, kn, dims[0], dims[1])];   TSym1 = TS[mindex3(in, jn, kn, dims[0], dims[1])];
                //if (IsInf(TSym1)) {
                //cout << "TSym1-location-1 = " << TSym1 << endl;
		//}
	} else {
		Tym1 = INF;  TSym1 = INF;
		//if (IsInf(TSym1)) {
                //cout << "TSym1-location-2 = " << TSym1 << endl;
                //}
	}
	in = i + 0; jn = j + 1; kn = k + 0; if (isfrozen3d(in, jn, kn, dims, Frozen)) {
	...
	for (t = 0; t < 3; t++) {    // t = 0, 1, 2. indicates that we only use the horizontal and vertical directions.   -wdd
			...
			Coeff_ts[0] += (2.2500)*(abs(Tt - Tm2[t])) * (1 / (dzxy * dzxy));  Coeff_ts[1] += -1 * (2.2500) * (abs(Tt - Tm2[t])) * (1 / (dzxy * dzxy)) * TSm2[t];
			//cout << "Coeff_ts[0] = " << Coeff_ts[0] << endl;
                        break;
                }
        }

        //cout << "(Tt - Tm[0]) * (1 / (dzxy * dzxy)) = " << (Tt - Tm[0]) * (1 / (dzxy * dzxy)) << endl;
	//cout << "(Tt - Tm[1]) * (1 / (dzxy * dzxy)) = " << (Tt - Tm[1]) * (1 / (dzxy * dzxy)) << endl;
	//cout << "(Tt - Tm[2]) * (1 / (dzxy * dzxy)) = " << (Tt - Tm[2]) * (1 / (dzxy * dzxy)) << endl;

	//cout << "(2.2500)*(Tt - Tm2[0]) * (1 / (dzxy * dzxy)) = " << (2.2500)*(Tt - Tm2[0]) * (1 / (dzxy * dzxy)) << endl;
        //cout << "(2.2500)*(Tt - Tm2[1]) * (1 / (dzxy * dzxy)) = " << (2.2500)*(Tt - Tm2[1]) * (1 / (dzxy * dzxy)) << endl;
	//cout << "(2.2500)*(Tt - Tm2[2]) * (1 / (dzxy * dzxy)) = " << (2.2500)*(Tt - Tm2[2]) * (1 / (dzxy * dzxy)) << endl;

        //cout << "Tt - Tm2[0] = " << Tt - Tm2[0] << endl;
	//cout << "Tt - Tm2[1] = " << Tt - Tm2[1] << endl;
	//cout << "Tt - Tm2[2] = " << Tt - Tm2[2] << endl;

	TSt = -1 * Coeff_ts[1] / Coeff_ts[0];

	//cout << "Coeff_ts[0] = " << Coeff_ts[0] << endl;
	//cout << "Coeff_ts[1] = " << Coeff_ts[1] << endl;

	//if (isnan(TSt)) {
        //if (TSt < 1.0e-5) {

		//cout << "TSym1 = " << TSym1 << endl;
		//cout << "TSym2 = " << TSym2 << endl;

		//cout << "TSym2-TSym1 = " << TSym2-TSym1 << endl;

		//cout << "TSyp1 = " << TSyp1 << endl;
		//cout << "TSyp2 = " << TSyp2 << endl;

		//cout << "TSm2[1] = " << TSm2[1] << endl;
