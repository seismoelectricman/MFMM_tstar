		...
		for (k = 0; k < 4; k++) {
			...
			if (isntfrozen2d(i, j, dims, Frozen)) {
				...
				if (abs(izsrcn - i) <= bz && abs(ixsrcn - j) <= bx) {
					...
					CalculateDis2DResult result = CalculateDistance2D(T, TS, F[IJ_index], Q[IJ_index], dims, i, j, usesecond, usecross, Frozen);
				...
				if (Ed) {
					...
					CalculateDis2DResult result = CalculateDistance2D(Y, Y, 1, 1, dims, i, j, usesecond, usecross, Frozen);
					Tt = result.Tt; TSt = result.TSt;
				}
			...
			if (isntfrozen3d(i, j, k, dims, Frozen)) {
				...
				if (T[IJK_index] > 0) {  // At beginning, T is '-1'. -wdd
					...
					if (neg_pos >= neg_free) {  // indicates that the total number of node within the vector, which then can be used to select min values. -wdd 
						neg_free += 100000; // At beginning, neg_free = 100000; // -wdd
		...
		for (w = 0; w < 6; w++) { // 0, 1, 2, 3, 4, 5. -wdd
			...
			if (isntfrozen3d(i, j, k, dims, Frozen)) {
				...
				if (abs(izsrcn - i) <= bz && abs(ixsrcn - j) <= bx && abs(iysrcn - i) <= by) {
					...
					// should multiply the dimension of the grid - Dongzhuo
					//Tt = CalculateDistance3D(T, F[IJK_index], dims, i, j, k, usesecond, usecross, Frozen);
					CalculateDis3DResult result = CalculateDistance3D(T, TS, F[IJK_index], Q[IJK_index], dims, i, j, k, usesecond, usecross, Frozen);
				...
				if (Ed) {
					...
					CalculateDis3DResult result = CalculateDistance3D(Y, Y, 1, 1, dims, i, j, k, usesecond, usecross, Frozen);
					Tt = result.Tt; TSt = result.TSt;
					//cout << "Tt/TSt = " << Tt/TSt << endl;
				}
				...
				if ((T[IJK_index] > -1) && T[IJK_index] <= listproptt[0]) {  // T[IJK_index] > -1 - indicates that this node has an existing Tt (T is not -1).
					...
					/*If running out of memory at a new block */
					if (neg_pos >= neg_free) {
						neg_free += 100000; // If it exceeds 100000, then we allocate another 100000. -wdd
						neg_listx = (double *)realloc(neg_listx, neg_free * sizeof(double) );
						neg_listy = (double *)realloc(neg_listy, neg_free * sizeof(double) );
						neg_listz = (double *)realloc(neg_listz, neg_free * sizeof(double) );
						if (Ed) {
...
CalculateDis3DResult CalculateDistance3D(double *T, double *TS, double Fijk, double Qijk, int *dims, int i, int j, int k, bool usesecond, bool usecross, bool *Frozen) {
	...
	if (usesecond) {
		...
		Tm2[0] = second_derivative(Txm1, Txm2, Txp1, Txp2);  TSm2[0] = second_derivative(TSxm1, TSxm2, TSxp1, TSxp2);
		if (IsInf(Tm2[0])) {
			Tm2[0] = 0;   TSm2[0] = 0;
		} else {
			Order[0] = 2; // Order[0] = 2;
		...
		Tm2[1] = second_derivative(Tym1, Tym2, Typ1, Typ2);  TSm2[1] = second_derivative(TSym1, TSym2, TSyp1, TSyp2);
		if (IsInf(Tm2[1])) {
			Tm2[1] = 0;   TSm2[1] = 0;
		} else {
			Order[1] = 2; // Order[1] = 2; 
		...
		Tm2[2] = second_derivative(Tzm1, Tzm2, Tzp1, Tzp2);  TSm2[2] = second_derivative(TSzm1, TSzm2, TSzp1, TSzp2);
		if (IsInf(Tm2[2])) {
			Tm2[2] = 0;   TSm2[2] = 0;
		} else {
			Order[2] = 2;  // Order[2] = 2;
